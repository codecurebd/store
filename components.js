...
<head>
  ...
  <title>CodeCureBD — Cart Popup</title>
  ...
  <style>
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .cart-dropdown-header .cart-count { font-size:0.8rem; font-weight:400; color:#94a3b8; }
    .cart-dropdown-items { max-height:320px; overflow-y:auto; padding:4px 0; }
    .cart-dropdown-items::-webkit-scrollbar { width:4px; }
    .cart-dropdown-items::-webkit-scrollbar-thumb { background:#cbd5e1; border-radius:4px; }
    ...
    .cart-item img { width:48px; height:48px; object-fit:cover; border-radius:10px; flex-shrink:0; background:#f1f5f9; }
    .cart-item-info { flex:1; min-width:0; }
    .cart-item-info .name { font-weight:500; color:#0f172a; font-size:0.9rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .cart-item-info .price { font-size:0.85rem; color:#475569; font-weight:500; }
    .cart-item .remove-btn {
      width:30px; height:30px; border-radius:50%; border:none; background:transparent;
      color:#94a3b8; font-size:1rem; cursor:pointer; transition:all 0.2s;
      display:flex; align-items:center; justify-content:center; flex-shrink:0;
    }
    .cart-item .remove-btn:hover { background:#fee2e2; color:#ef4444; }

    .cart-dropdown-footer { padding:16px 20px 20px; border-top:1px solid #f1f5f9; }
    .cart-total-row {
      display: flex;
      justify-content: space-between;
      font-weight: 700;
    ...
    .cart-total-row .total-amount { color:#2563eb; }
    .cart-dropdown-footer .btn-primary { width:100%; justify-content:center; padding:12px; font-size:0.95rem; }
    .cart-empty { padding:36px 20px; text-align:center; color:#94a3b8; }
    .cart-empty i { font-size:2.4rem; opacity:0.3; margin-bottom:8px; display:block; }
    ...
    .toast {
      ...
      pointer-events:auto; border-left:4px solid #007aff;
      width:100%;
    }
    ...
    /* ── Demo content ── */
    .demo-content { max-width:800px; margin:40px auto; padding:0 24px; text-align:center; }
    .demo-content h1 { font-size:2rem; font-weight:700; color:#0f172a; margin-bottom:12px; }
    .demo-content p { color:#475569; max-width:500px; margin:0 auto 32px; }
    .demo-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:20px; margin-top:20px; }
    .demo-card { background:#fff; border-radius:16px; padding:20px; box-shadow:0 4px 16px rgba(0,0,0,0.06); border:1px solid #f1f5f9; }
    .demo-card img { width:100%; height:120px; object-fit:cover; border-radius:10px; background:#f1f5f9; margin-bottom:10px; }
    .demo-card h4 { color:#0f172a; font-size:1rem; }
    .demo-card .price-tag { color:#2563eb; font-weight:700; font-size:1.1rem; margin:4px 0 10px; }
    .demo-card .btn-primary { font-size:0.8rem; padding:8px 16px; }
    ...
    .dropdown-menu hr { margin:4px 0; border-color:#f1f5f9; }
    ...
    @media (max-width:480px) {
      .cart-dropdown { width:92vw; right:4vw; top:calc(100% + 6px); }
...
<body>
  ...
  <div class="demo-content">
    <h1>🛍️ Popup Cart Demo</h1>
    <p>Click the cart icon <i class="fas fa-shopping-cart"></i> in the navbar. No sidebar, no quantity — just a clean dropdown.</p>
    <div class="demo-grid" id="productGrid"></div>
  </div>
  ...
  <script>
    // ─────────────────────────────────────────────────────────────────
    //  FIREBASE MOCK (replace with your real config in production)
    // ─────────────────────────────────────────────────────────────────
    const auth = {
    ...
    const signOut = () => { auth.currentUser = null; updateNavbarAuth(null); showToast('Logged out', 'info'); };
    ...
    // ─────────────────────────────────────────────────────────────────
    //  CART DROPDOWN — NEW (no sidebar, no quantity)
    // ─────────────────────────────────────────────────────────────────
    ...
    function renderPaymentModal() {
      ...
      if (!document.getElementById('paymentModalStyle')) {
        ...
        style.textContent = `@keyframes scaleIn { from { opacity:0; transform:scale(0.95); } to { opacity:1; transform:scale(1); } }`;
    ...
    console.log('✅ All systems ready — cart popup active.');
  </script>
